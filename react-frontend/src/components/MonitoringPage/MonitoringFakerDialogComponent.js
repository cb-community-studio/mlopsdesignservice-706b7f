...
import monitoringFakerFactory from "./monitoringFakerFactory";
...
const MonitoringFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = monitoringFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("monitoring").create(elem));
...
export default connect(null, mapDispatch)(MonitoringFakerDialogComponent);
