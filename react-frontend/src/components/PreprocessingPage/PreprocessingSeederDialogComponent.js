...
import seederData from "./preprocessing.seeder.json";
...
const PreprocessingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("preprocessing").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(PreprocessingSeederDialogComponent);
