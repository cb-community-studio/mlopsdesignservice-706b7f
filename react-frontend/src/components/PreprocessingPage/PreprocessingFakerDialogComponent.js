...
import preprocessingFakerFactory from "./preprocessingFakerFactory";
...
const PreprocessingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = preprocessingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("preprocessing").create(elem));
...
export default connect(null, mapDispatch)(PreprocessingFakerDialogComponent);
