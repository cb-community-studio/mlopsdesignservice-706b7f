...
import visualizationFakerFactory from "./visualizationFakerFactory";
...
const VisualizationFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = visualizationFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("visualization").create(elem));
...
export default connect(null, mapDispatch)(VisualizationFakerDialogComponent);
