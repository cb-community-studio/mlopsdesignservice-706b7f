...
import dockerFakerFactory from "./dockerFakerFactory";
...
const DockerFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = dockerFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("docker").create(elem));
...
export default connect(null, mapDispatch)(DockerFakerDialogComponent);
