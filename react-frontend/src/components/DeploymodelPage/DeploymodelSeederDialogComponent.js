...
import seederData from "./deploymodel.seeder.json";
...
const DeploymodelSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("deploymodel").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(DeploymodelSeederDialogComponent);
