...
import deploymodelFakerFactory from "./deploymodelFakerFactory";
...
const DeploymodelFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = deploymodelFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("deploymodel").create(elem));
...
export default connect(null, mapDispatch)(DeploymodelFakerDialogComponent);
