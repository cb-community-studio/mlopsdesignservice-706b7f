...
import modeltrainingFakerFactory from "./modeltrainingFakerFactory";
...
const ModeltrainingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = modeltrainingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("modeltraining").create(elem));
...
export default connect(null, mapDispatch)(ModeltrainingFakerDialogComponent);
