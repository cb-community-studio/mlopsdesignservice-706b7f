...
import seederData from "./modeltraining.seeder.json";
...
const ModeltrainingSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("modeltraining").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(null, mapDispatch)(ModeltrainingSeederDialogComponent);
