...
import thresholdFakerFactory from "./thresholdFakerFactory";
...
const ThresholdFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = thresholdFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("threshold").create(elem));
...
export default connect(null, mapDispatch)(ThresholdFakerDialogComponent);
