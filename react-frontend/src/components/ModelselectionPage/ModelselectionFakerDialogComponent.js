...
import modelselectionFakerFactory from "./modelselectionFakerFactory";
...
const ModelselectionFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = modelselectionFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("modelselection").create(elem));
...
export default connect(null, mapDispatch)(ModelselectionFakerDialogComponent);
