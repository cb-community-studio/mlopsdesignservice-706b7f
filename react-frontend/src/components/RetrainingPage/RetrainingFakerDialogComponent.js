...
import retrainingFakerFactory from "./retrainingFakerFactory";
...
const RetrainingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = retrainingFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("retraining").create(elem));
...
export default connect(null, mapDispatch)(RetrainingFakerDialogComponent);
