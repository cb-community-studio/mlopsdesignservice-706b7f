...
import datasourceFakerFactory from "./datasourceFakerFactory";
...
const DatasourceFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = datasourceFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("datasource").create(elem));
...
export default connect(null, mapDispatch)(DatasourceFakerDialogComponent);
